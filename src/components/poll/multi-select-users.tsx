...
export function UserMultiSelect({ users, selectedUsers, onSelectedUsersChange, maxSelection }: UserMultiSelectProps) {
  ...
  const selectableUsers = users.filter(u => u.isVerified);
            ...
            <CommandList>
              ...
              <CommandGroup>
                {selectableUsers.map(user => {
                  const isSelected = selectedUsers.some(u => u.uid === user.uid);
                  const isMaxedOut = maxSelection && selectedUsers.length >= maxSelection && !isSelected;
                  return (
                    <CommandItem
                      key={user.uid}
                      onSelect={() => handleSelect(user)}
                      disabled={isMaxedOut}
                      className={cn("flex items-center justify-between", isMaxedOut && "opacity-50 cursor-not-allowed")}
                    >
