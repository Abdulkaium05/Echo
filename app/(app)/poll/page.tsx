...
import { getNormalUsers, type UserProfile } from '@/services/firestore';
...
export default function PollPage() {
  ...
  useEffect(() => {
    if (userProfile?.isDevTeam) {
      getNormalUsers()
        .then(users => {
            // The getNormalUsers function already excludes the current user, bot, and dev team.
            setAllUsers(users);
        })
    ...
    <div className="container mx-auto py-8 px-4 md:px-6 lg:px-8 h-full flex items-center justify-center">
      <Card className="w-full max-w-lg">
        <CardHeader>
          ...
          <CardDescription>
            Select 2 to 4 users from the app and ask a question to the community.
          </CardDescription>
        </CardHeader>
